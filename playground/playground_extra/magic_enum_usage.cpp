...
enum class Flags { Read = 1 << 0, Write = 1 << 1, Execute = 1 << 2 };
...
struct magic_enum::customize::enum_name<Flags> {
  static constexpr std::string_view name(Flags value) {
    switch (value) {
      ...
      default:
        return {};  // Let the default behavior handle other values
    }
  }
};
...
constexpr auto numbers =
    magic_enum::enum_values<Numbers, Numbers::Two, Numbers::Four>();
...
int main() {
  ...
  if (blue.has_value()) {
    std::cout << "Converted string to enum: " << static_cast<int>(blue.value())
              << std::endl;  // 2
  }
  ...
  // Count
  std::cout << "Number of colors: " << magic_enum::enum_count<Color>()
            << std::endl;  // 3
