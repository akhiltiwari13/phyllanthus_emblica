...
enum class ConfigStorageType { HEAP, STACK };

// Function to load YAML config and store it on the heap or stack
void loadAndPrintConfig(const std::string& filename,
...
int main(int argc, char** argv) {
  if (argc < 3) {
    std::cout << "Usage: " << argv[0] << " <config_file.yaml> <storage_type>"
              << std::endl;
  ...
  if (storageArg == "heap") {
    ...
    std::cout << "Unknown storage type '" << storageArg
              << "'. Use 'heap' or 'stack'." << std::endl;
    return 1;
  }
