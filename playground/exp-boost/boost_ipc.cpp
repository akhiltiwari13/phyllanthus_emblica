...
int main() {
  shared_memory_object shdmem{open_or_create, "quomptrade-test.shm",
                              read_write};
  shdmem.truncate(1024);
  std::cout << shdmem.get_name() << '\n';
  offset_t size;
  if (shdmem.get_size(size)) std::cout << size << '\n';

  // memory mapped region demo
  // Because shared memory is used to exchange data between different processes,
  // each process needs to map the shared memory into its address space. The
  // class boost::interprocess::mapped_region is used to do this.
