...
long long totalLiquidFilled(vector<vector<int>> matrix) {
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      if (j == 0)
        leftMax[i][j] = matrix[i][j];
      else
        leftMax[i][j] = max(leftMax[i][j - 1], matrix[i][j]);

      if (i == 0)
        topMax[i][j] = matrix[i][j];
      else
        topMax[i][j] = max(topMax[i - 1][j], matrix[i][j]);
    }
  }
  ...
  for (int i = rows - 1; i >= 0; i--) {
    for (int j = cols - 1; j >= 0; j--) {
      if (j == cols - 1)
        ...
        rightMax[i][j] = max(rightMax[i][j + 1], matrix[i][j]);

      if (i == rows - 1)
        bottomMax[i][j] = matrix[i][j];
      else
        bottomMax[i][j] = max(bottomMax[i + 1][j], matrix[i][j]);
    }
  }
  ...
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      int minHeight =
          min({leftMax[i][j], rightMax[i][j], topMax[i][j], bottomMax[i][j]});
  ...
  /*    for (int i = 0; i < rows; i++) {
      for (int j = 0; j < cols; j++) {
          cout << "LeftMax: " << leftMax[i][j] << ", RightMax: " <<
  rightMax[i][j]
               << ", TopMax: " << topMax[i][j] << ", BottomMax: " <<
  bottomMax[i][j] << endl;
      }
  }
