...
int totalLiquidFilled(vector<vector<int>> matrix) {
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      if (i == 0)
        ...
        top[i][j] = max(top[i - 1][j], matrix[i][j]);

      if (j == 0)
        left[i][j] = matrix[i][j];
      else
        left[i][j] = max(left[i][j - 1], matrix[i][j]);
    }
  }
  ...
  for (int i = m - 1; i >= 0; i--) {
    for (int j = n - 1; j >= 0; j--) {
      if (i == m - 1)
        ...
        bottom[i][j] = max(bottom[i + 1][j], matrix[i][j]);

      if (j == n - 1)
        right[i][j] = matrix[i][j];
      else
        right[i][j] = max(right[i][j + 1], matrix[i][j]);
    }
  }
  ...
  for (int i = 0; i < m; i++) {
    for (int j = 0; j < n; j++) {
      total += min({top[i][j], bottom[i][j], left[i][j], right[i][j]}) -
               matrix[i][j];
    }
  }
  return total;
