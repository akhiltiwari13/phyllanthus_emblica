...
// The struct contains its column number, row number, a right and a down
// pointer, and its value. The actual matrix nodes are one indexed while the
// margin ones have either coordinate equal to zero. In c++, nodes can also have
// a constructor as used below
typedef struct node {
  int colno;
  int rowno;
  node* right;
...
void insertIntoResultMatrix(node* result, int row, int col, int value) {
  ...
  while (current != NULL && current->colno < col) current = current->right;
  ...
  while (rowP != NULL) {
    ...
    while (colQ != NULL) {
      ...
      if (sum != 0) {
        ...
        // You would need a function to insert a new value into the result
        // sparse matrix This function is not implemented here
...
int main() {
  ...
  while (matno != -1) {
    ...
    if (rowsl[matno]->rowno > i) rowsl[matno] = &root[matno];
    if (topsl[matno]->colno > j) topsl[matno] = &root[matno];
    while (rowsl[matno]->rowno != i) rowsl[matno] = rowsl[matno]->down;
    while (topsl[matno]->colno != j) topsl[matno] = topsl[matno]->right;
    node *dwn = topsl[matno], *rgt = rowsl[matno];
    while (dwn->down != NULL && dwn->down->rowno < i) dwn = dwn->down;
    while (rgt->right != NULL && rgt->right->colno < j) rgt = rgt->right;
