...
FuncStatus_t TransformData(const size_t inputDataLength,
...
const char* FuncStatusString[4] = {"STATUS_SUCCESS", "STATUS_BAD_POINTER",
                                   "STATUS_BAD_ARRAY_LENGTH",
                                   "STATUS_NO_SENSOR_DATA"};
...
int main() {
  ...
  expectedOutputDataLength =
      sizeof(uint32_t) * (inputDataLength - 1) + sizeof(uint8_t);
  ...
  result =
      TransformData(inputDataLength, null_input_array ? NULL : inputDataValues,
                    outputDataLength, outputDataValues);
...
exit:
  if (inputDataValues != NULL) free(inputDataValues);

  if (outputDataValues != NULL) free(outputDataValues);
