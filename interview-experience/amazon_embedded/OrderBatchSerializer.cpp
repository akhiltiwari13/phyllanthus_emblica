...
//     size_t serial_sz = sizeof(OrderBatch) - sizeof(Order*) +
//     ((ob->order_count)*sizeof(Order)); return serial_sz;
// }

// FuncStatus_t serialize_order(const struct OrderBatch *order_batch, const
// size_t out_max_length, uint8_t *out) {
//     /* Warning: You can use stdout for debugging, but doing so will cause the
//     test cases to fail. */ auto tptr{out}; int_fast32_t payload_len{}; auto
//     face = 0xFACE; if(!order_batch) return STATUS_NULL_INPUT; else{
//         //process a valid orderbatch;
//         payload_len = getSerialSize(order_batch) + 10;
//         if(payload_len < out_max_length) return
//         STATUS_INSUFFICIENT_OUTPUT_BUFFER;
...
//             //memcpy(tptr,&ords,sizeof(Order)); //serialize order_count &
//             batch_id
//             //tptr+=sizeof(Order);
...
// TODO: test this code.
// Note that this implementation assumes that the output buffer has already been
// allocated and is of the correct size. It also assumes that the data types
// used in the OrderBatch and Order structures have the expected sizes and
// endianness. To make the serialization more robust and platform-independent,
// one could consider using fixed-size data types from the <cstdint> header
// and/or adding code to convert endianness if necessary.

// bool serialize_order(const OrderBatch *order_batch, const size_t
// out_max_length, uint8_t *out) {
// // check if output buffer is large enough to store serialized data
// size_t expected_size = sizeof(OrderBatch) + sizeof(Order) *
// order_batch->order_count; if (expected_size > out_max_length) { return false;
// }
...
FuncStatus_t serialize_order(const OrderBatch* order_batch,
                             const size_t out_max_length, uint8_t* out) {
  if (!order_batch) {
    return STATUS_NULL_INPUT;
  }

  size_t payload_len =
      10 + 8 + 4 + order_batch->order_count * (2 + 8 + 16 + 32);
...
int read_in_order(struct Order* order) {
  ...
  fgets(line, 1024, stdin);
  if (16 != sscanf(line,
                   "%hhu %hhu %hhu %hhu %hhu %hhu %hhu %hhu %hhu %hhu %hhu "
                   "%hhu %hhu %hhu %hhu %hhu",
                   &part_number[0], &part_number[1], &part_number[2],
                   &part_number[3], &part_number[4], &part_number[5],
                   &part_number[6], &part_number[7], &part_number[8],
                   &part_number[9], &part_number[10], &part_number[11],
                   &part_number[12], &part_number[13], &part_number[14],
                   &part_number[15])) {
    printf("Error occured at line %d of test code.\n", __LINE__);
    return 3;
  }
