...
template <typename T, uint64_t SIZE = 4096,
          uint64_t MAX_SPIN_ON_BUSY = 40000000>
class ConcurrentQueue {
 private:
  // review note1: Instead of using a custom fuction to get log on base 2, the
  // standard library function can be used i.e std::log2
 ...
 public:
  ...
  bool peek() const { return (mWritePtr != mReadPtr); }

  // review note3: a data member could be used to return the size instead of
  ...
  void push(const T& pItem) {
    if (!busyWaitForPush()) {
      // review note4: this would abort the queue thus breaking the entire
      // messaging ecosystem.
