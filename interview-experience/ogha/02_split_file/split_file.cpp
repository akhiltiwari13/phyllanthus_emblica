...
struct Record {
  ...
  Record(string type_, string symbol_, string id_, string parentid_,
         string record_data_)
      : type(type_),
        symbol(symbol_),
        id(id_),
...
bool isParentRecord(const Record& record) { return record.type == "T"; }
...
void splitFile(const std::string& filePath, int X) {
  ...
  for (const auto& rcrds : parentChildRecordLookup) {
    if ((recordCount == 0 || recordCount >= X) &&
        totalRecordsToWrite >= X) {  // check for creating a new output file!
      if (outFile.is_open()) {
        outFile.close();
      }
...
int main(int argc, char* argv[]) {
  if (argc != 3) {
    std::cerr << "Usage: " << argv[0]
              << " <file path> <minimum number of records in a file>\n";
    return 1;
  }
