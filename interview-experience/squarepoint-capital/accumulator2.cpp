...
struct accumulator<T, Op>::pimpl {
 public:
  ...
  void push(T value) {
    ...
    state_ = op_(std::move(state_), std::move(value));
  }
...
bool check_003() {
  if constexpr (check_003_enabled) {
    auto acc = accumulator(std::make_unique<int>(10), [](auto lhs, auto rhs) {
...
bool check_005() {
  if constexpr (check_005_enabled) {
    ...
    auto acc =
        accumulator(0.0, [count = 0, sum = 0.0](auto val, auto rhs) mutable {
          val = val + 0;
          sum += rhs;
          count++;
          return sum / count;
