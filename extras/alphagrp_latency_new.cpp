...
int main() {
  ...
  for (auto& reqPair : requestMap) {
    ...
    for (size_t i = 0; i < reqList.size(); ++i) {
      if (i < resList.size()) {
        ...
        ackDataList.push_back({reqList[i].timestamp, latency,
                               reqList[i].subType, reqList[i].symbol});
      }
    }
  }

  // Sort the ack data list by request timestamp
  std::sort(ackDataList.begin(), ackDataList.end(),
  ...
  for (const auto& ack : ackDataList) {
    std::cout << ack.requestTimestamp << ',' << ack.ackLatency << ','
              << ack.subType << ',' << ack.symbol << std::endl;
  }
