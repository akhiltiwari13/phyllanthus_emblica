...
long long safe_stoll(const std::string& str) {
    try {
        ...
        std::cerr << "Conversion error: " << e.what() << " for string: " << str << std::endl;
        return 0;
    }
}
...
int main() {
    ...
    while (std::getline(std::cin, line) && !line.empty()) {
        ...
        if (isRequest) {
            ...
            if (it != requests.end()) {
                AckData ack = {it->second.timestamp, timestamp - it->second.timestamp, it->second.subType, it->second.symbol, orderId};
    ...
    std::sort(latencies.begin(), latencies.end(), [](const AckData& a, const AckData& b) {
    ...
    for (const auto& ack : latencies) {
        std::cout << ack.requestTimestamp << ',' << ack.ackLatency << ',' << ack.subType << ',' << ack.symbol << ',' << ack.orderId << std::endl;
    }
